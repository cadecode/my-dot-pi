...
| 包 | 用途 | 验证命令 |
|----|------|---------|
| `pandas` | 数据处理与统计分析 | `<skill-dir>/.venv/Scripts/python -c "import pandas; print(pandas.__version__)"` |
| `matplotlib` | 图表绘制 | `<skill-dir>/.venv/Scripts/python -c "import matplotlib; print(matplotlib.__version__)"` |
| `seaborn` | 统计可视化 | `<skill-dir>/.venv/Scripts/python -c "import seaborn; print(seaborn.__version__)"` |

> Python 依赖使用 uv 管理。

## Files

- `analyze.py` - 核心分析脚本（调用方式：`{baseDir}/.venv/Scripts/python {baseDir}/analyze.py <csv_path>`）

## Usage

```python
from analyze import summarize_csv
result = summarize_csv("path/to/file.csv")
print(result)
```
